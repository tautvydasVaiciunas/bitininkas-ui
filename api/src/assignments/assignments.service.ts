...
export class AssignmentsService {
  ...
  async create(dto: CreateAssignmentDto, user) {
    ...
    const hive = await this.hiveRepository.findOne({ where: { id: dto.hiveId } });
    if (!hive) {
    ...
    await this.activityLog.log('assignment_created', user.id, 'assignment', saved.id);
    return saved;
  }
  ...
  async findAll(filter: { hiveId?: string }, user) {
    ...
    if (user.role === UserRole.USER) {
      const hiveIds = await this.hiveRepository.find({
        where: { ownerUserId: user.id },
        select: ['id'],
      });
      const ids = hiveIds.map((h) => h.id);
      if (filter.hiveId) {
        if (!ids.includes(filter.hiveId)) {
          return [];
        }
      } else {
        if (!ids.length) {
  ...
  async getDetails(id: string, user) {
    const assignment = await this.findOne(id, user);
    const task = await this.taskRepository.findOne({ where: { id: assignment.taskId }, relations: ['steps'] });
    if (!task) {
      throw new NotFoundException('Task not found');
    }
    const progress = await this.progressRepository.find({ where: { assignmentId: id } });
    const completedStepIds = new Set(progress.map((p) => p.taskStepId));
    const totalSteps = task?.steps?.length || 0;
    const completedSteps = totalSteps ? task.steps.filter((step) => completedStepIds.has(step.id)).length : 0;
  ...
  async calculateHiveSummary(hiveId: string, user) {
    ...
    if (user.role === UserRole.USER && hive.ownerUserId !== user.id) {
      throw new ForbiddenException('Access denied');
    }
    const assignments = await this.assignmentsRepository.find({ where: { hiveId } });
    if (assignments.length === 0) {
      return { hiveId, assignmentsCount: 0, completion: 0 };
    }
    const assignmentIds = assignments.map((a) => a.id);
    const progress = await this.progressRepository.find({ where: { assignmentId: In(assignmentIds) } });
    const completedStepIds = new Set(progress.map((p) => p.taskStepId));
    let totalSteps = 0;
    let completed = 0;
    for (const assignment of assignments) {
      const taskSteps = await this.stepRepository.find({ where: { taskId: assignment.taskId } });
      totalSteps += taskSteps.length;
      completed += taskSteps.filter((step) => completedStepIds.has(step.id)).length;
    }
    const percent = totalSteps === 0 ? 0 : Math.round((completed / totalSteps) * 100);
    return { hiveId, assignmentsCount: assignments.length, completion: percent };
  }
}
