...
const AdminSupport = () => {
  ...
  const loadThreadMessages = useCallback(async (threadId: string, cursor?: string, appendOlder = false) => {
    ...
    try {
      ...
      if (!appendOlder) {
        setMessagesError('Nepavyko ikelti žinuciu.');
      }
    } finally {
      if (appendOlder) {
    ...
    <MainLayout>
      <div className="mx-auto flex w-full max-w-6xl gap-6 py-10 px-4">
            ...
            threads.map((thread) => (
              ...
              >
                ...
                <p className="text-xs text-muted-foreground">
                  {thread.lastMessageText ?? 'Nera žinuciu'} • {thread.unreadFromUser} neperskaitytu
                </p>
              </button>
            ))
          )}
        ...
        <div className="flex-1 space-y-4 rounded-2xl border border-border bg-background/80 p-4 shadow-sm shadow-black/5">
          <header>
            <h1 className="text-xl font-semibold">Žinutes</h1>
          ...
          <div className="max-h-[60vh] space-y-4 overflow-y-auto rounded-xl border border-border/70 p-3">
            ...
            {messagesLoading ? (
              ...
              <p className="text-center text-sm text-muted-foreground">Pasirinkite thread'a arba parašykite pirma žinute.</p>
            ) : (
